...
class ilObjNoticeboardGUI extends ilObjectPluginGUI
{
	...
	{
		return ($this->checkOwner() || $this->rbac->review()->isAssigned($this->user->getId(), SYSTEM_ROLE_ID));
	}
			...
			default:
				switch($cmd)
				{
					...
					case 'setFilterBySubtab':
						$this->tabs->activateTab($_SESSION['activeTab']);
					...
					case 'showMyNotices':
						...
						$this->tabs->activateTab('showMyNotices');
						if($this->getPermission('read'))
						{
							$cmd .= 'Action';
							$this->$cmd();
					...
					case 'saveCategoryPermissions':
						$this->tabs->activateTab('permissions');
						$this->$cmd();
						break;
	...
	protected function initCategoriesSubtabs($a_tab)
	{
		...
		foreach(ilNoticeCategory::getList($this->object->getId()) as $category)
		{
			...
			if($a_tab == 'showMyNotices')
			{
				if(!ilNoticeboardObjPermission::hasWriteAccess($this->user->getId(), $category->getCategoryId()))
				{
	...
	protected function editProperties()
	{
		...
		{
			ilUtil::sendInfo($this->pluginObj->txt('please_create_categories'), true);
			$this->showCategories();
		}
	...
	protected function updateCategories()
	{
		...
		if($this->formGui->checkInput())
		{
			...
			else
			{
				...
				$objPermissions = new ilNoticeboardObjPermission();
	...
	protected function showBoardAction()
	{
		...
		$this->noticeRepository = new ilNoticeRepository($this->object);
		ilNotice::performValidityChecks();
		...
		if($this->isFilterEnabled())
		{
			...
			else
			{
				...
				if((int)$_SESSION['xnob_filter']['cat_id'] > 0 && ilNoticeboardObjPermission::hasWriteAccess($this->user->getId(), (int)$_SESSION['xnob_filter']['cat_id']))
				{
		...
		// check category permissions
		if($this->currentCategoryId > 0 && ilNoticeboardObjPermission::hasWriteAccess($this->user->getId(), $this->currentCategoryId))
		{
			$table->setWriteAccess(true);
		}
	...
	protected function buildNoticeDataArray(array $notices)
	{
		...
		foreach($notices as $notice)
		{
			...
			{
				$data[$i]['image']     = $this->pluginObj->getDirectory() . '/templates/images/icon_no_image.svg';
	...
	protected function showAction($show_js_box = true)
	{
		...
		if($this->ctrl->getCmd() == 'showMyNotice')
		{
		...
		// initialize and check directories
		...
		if($show_js_box == true)//&& ($notice->getUserId() != $this->user->getId()))
		{
			...
			foreach(array(
				        ...
				        'notice_recommended_successfully', 'notice_reported_successfully', 'recipient_missing'
			        ) as $lng_key)
			{
		...
		$this->toolbar->addButton($this->lng->txt('back'), $this->ctrl->getLinkTarget($this, $cmd));
		...
		if(($notice->getUserId() != $this->user->getId()) && $show_js_box == true)
		{
			...
			$this->ctrl->setParameter($this, 'notice_id', $notice->getId());
			$this->toolbar->addButton($this->txt('contact_user'), $this->ctrl->getLinkTarget($this, 'contact'));
		}
	...
	protected function deleteAction()
	{
		...
		$this->ctrl->setParameter($this, 'category_id', $this->currentCategoryId);
		$this->ctrl->redirectToURL($this->ctrl->getLinkTarget($this, $cmd, '', FALSE, FALSE));
	}
	...
	protected function moveNoticeAction()
	{
		...
		$select = new ilSelectInputGUI($this->txt('move_to'), 'category_id');
		...
		if($_POST['action'] == 'send')
		{
			...
			if($this->formGui->checkInput())
			{
				...
				else
				{
					...
					$this->ctrl->setParameter($this, 'notice_id', $this->notice->getId());
					$this->ctrl->setParameter($this, 'tab', $_GET['tab']);
					$this->ctrl->redirectToURL($this->ctrl->getLinkTarget($this, 'update', '', FALSE, FALSE));
				}
			}
		}
		$this->showForm();
	...
	protected function showForm()
	{
		...
		$this->toolbar->addButton($this->lng->txt('back'), $this->ctrl->getLinkTarget($this, $_SESSION['activeTab']));
				...
				if($recipient != '')
				{
					...
					if($_POST['action'] == 'send')
					{
						...
						if($this->formGui->checkInput())
						{
							...
							$this->ctrl->redirectToURL($this->ctrl->getLinkTarget($this, 'show', '', FALSE, FALSE));
						}
					}
	...
	protected function showContactForm()
	{
		...
		$this->toolbar->addButton($this->lng->txt('back'), $this->ctrl->getLinkTarget($this, 'show', '', false, false));
	...
	public function showPermissions()
	{
		...
		$cat_permissions = ilNoticeboardObjPermission::getPermissionsByCatId($this->getCurrentCategoryId());
		
		$global_roles = $this->rbac->review()->getGlobalRoles($this->ref_id);
		$local_roles  = $this->rbac->review()->getLocalRoles($this->ref_id);
		
		$this->plugin->includeClass('class.ilNoticeboardPermissionsTableGUI.php');
		$tbl = new ilNoticeboardPermissionsTableGUI($this, 'showPermissions', $this->getCurrentCategoryId());
		...
		)
		{
			...
			$parent_roles  = $this->rbac->review()->getLocalRoles($parent_ref_id);
			$merge_roles   = array_merge($merge_roles, $parent_roles);
		}
		
		$this->ctrl->setParameter($this, 'category_id', $this->getCurrentCategoryId());
	...
	public function saveCategoryPermissions()
	{
		...
		$this->tabs->activateTab('categories');
		$this->plugin->includeClass('class.ilNoticeboardObjPermission.php');
		
		$objPermission = new ilNoticeboardObjPermission();
	...
	public function showFilter($in_my_notices = false)
	{
		...
		if($this->checkModeratePermission() || $in_my_notices == true)
		{
			...
			$form->addCommandButton('resetFilter', $this->lng->txt('reset_filter'));
			...
			$filter_status  = new ilSelectInputGUI($this->lng->txt('status'), 'filter_status');
